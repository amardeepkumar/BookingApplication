...
package com.helthifyme.bookingapplication.data;
...
import com.helthifyme.bookingapplication.dto.Time;
...
public class ExampleExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    public ExampleExpandableDataProvider(Day day) {

        final List<ChildData> morningTime = new ArrayList<>();
        final List<ChildData> afternoonTime = new ArrayList<>();
        final List<ChildData> eveningTime = new ArrayList<>();

        final ConcreteGroupData morning = new ConcreteGroupData(0, "Morning");
        final ConcreteGroupData afternoon = new ConcreteGroupData(1, "Afternoon");
        final ConcreteGroupData evening = new ConcreteGroupData(2, "Evening");

        final List<Time> morningTimeList = day.getMorning();
        morning.setChildCount(morningTimeList.size());
        for (Time time: morningTimeList) {
            time.setChildId(morning.generateNewChildId());
            time.setText(DateUtils.formatTimeSlot(time.getStartTime(), time.getEndTime()));
            morningTime.add(time);
        }

        final List<Time> afterNoonTimeList = day.getAfterNoon();
        afternoon.setChildCount(afterNoonTimeList.size());
        for (Time time: afterNoonTimeList) {
            time.setChildId(afternoon.generateNewChildId());
            time.setText(DateUtils.formatTimeSlot(time.getStartTime(), time.getEndTime()));
            afternoonTime.add(time);
        }

        final List<Time> eveningTimeList = day.getEvening();
        evening.setChildCount(eveningTimeList.size());
        for (Time time: eveningTimeList) {
            time.setChildId(evening.generateNewChildId());
            time.setText(DateUtils.formatTimeSlot(time.getStartTime(), time.getEndTime()));
            eveningTime.add(time);
        }

        mData = new LinkedList<>();
        mData.add(new Pair<GroupData, List<ChildData>>(morning, morningTime));
        mData.add(new Pair<GroupData, List<ChildData>>(afternoon, afternoonTime));
        mData.add(new Pair<GroupData, List<ChildData>>(evening, eveningTime));
    }
    ...
    public static final class ConcreteGroupData extends GroupData {
        ...
        public String getChildCount() {
            return mChildCount + " Slots available";
        }
    }
